...
func TestClusterStateManager_AddNode(t *testing.T) {
	node1 := &common.DataNodeInfo{ID: "node1", Host: "localhost", Port: 8080}
	node2 := &common.DataNodeInfo{ID: "node2", Host: "localhost", Port: 8081}

	tests := []struct {
		name            string
		nodeToAdd       *common.DataNodeInfo
		...
		},
		{
			...
			nodeToAdd:       &common.DataNodeInfo{ID: "node1", Host: "new-host"},
			initialNodes:    []*common.DataNodeInfo{node1},
			initialVersion:  1,
			expectedCount:   1,
			expectedVersion: 2,
...
func TestClusterStateManager_GetNode(t *testing.T) {
	...
	tests := []struct {
		...
		nodeID       string
		expectFound  bool
		expectedNode *common.DataNodeInfo
	}{
...
func TestClusterStateManager_ListNodes(t *testing.T) {
	...
	tests := []struct {
		...
		{"list with no nodes", []*common.DataNodeInfo{}, nil, 0, 0},
		{"list with one node", []*common.DataNodeInfo{node1}, nil, 1, 1},
		{"list with multiple nodes", []*common.DataNodeInfo{node1, node2}, nil, 2, 2},
		{"list with limit", []*common.DataNodeInfo{node1, node2}, []int{1}, 1, 2},
		{"list with limit greater than count", []*common.DataNodeInfo{node1, node2}, []int{5}, 2, 2},
	}
...
func TestClusterStateManager_RemoveNode(t *testing.T) {
	...
	tests := []struct {
		...
		{"remove existing node", "node1", []*common.DataNodeInfo{node1, node2}, 2, false, 1, 3},
		{"remove non-existent node", "node3", []*common.DataNodeInfo{node1, node2}, 2, true, 2, 2},
	}
...
func TestClusterStateManager_UpdateNode(t *testing.T) {
	...
	updatedNode1 := &common.DataNodeInfo{ID: "node1", Host: "updated-host"}

	tests := []struct {
		name            string
		nodeToUpdate    *common.DataNodeInfo
		initialNodes    []*common.DataNodeInfo
		initialVersion  int64
		expectErr       bool
		expectedVersion int64
		...
		{"update existing node", updatedNode1, []*common.DataNodeInfo{node1}, 1, false, 2, "updated-host"},
		{"update non-existent node", &common.DataNodeInfo{ID: "node2"}, []*common.DataNodeInfo{node1}, 1, true, 1, ""},
	}
...
func TestClusterStateManager_InitializeNodes(t *testing.T) {
	...
	tests := []struct {
		name            string
		nodesToInit     []*common.DataNodeInfo
		versionToSet    int64
		expectedCount   int
		expectedVersion int64
	}{
		{"initialize with empty list", []*common.DataNodeInfo{}, 5, 0, 5},
		{"initialize with nodes", []*common.DataNodeInfo{node1, node2}, 10, 2, 10},
	}
...
func TestClusterStateManager_ApplyUpdates(t *testing.T) {
	node1 := &common.DataNodeInfo{ID: "node1"}
	node2 := &common.DataNodeInfo{ID: "node2"}
	node1Updated := &common.DataNodeInfo{ID: "node1", Host: "updated"}
...
func TestClusterStateManager_GetCurrentVersion(t *testing.T) {
	...
	manager.AddNode(&common.DataNodeInfo{ID: "node1"})
	assert.Equal(t, int64(1), manager.GetCurrentVersion())

	manager.InitializeNodes([]*common.DataNodeInfo{}, 10)
...
func TestClusterStateManager_Concurrency(t *testing.T) {
	...
	for i := 0; i < numGoroutines; i++ {
		...
		go func(i int) {
			defer wg.Done()
			node := &common.DataNodeInfo{ID: fmt.Sprintf("node-%d", i)}
			manager.AddNode(node)
		}(i)
	}
	wg.Wait()
